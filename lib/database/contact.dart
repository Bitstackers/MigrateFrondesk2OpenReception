...
Future<adaheads_model.Contact> _getContact(Pool pool, int contactId) {
  String sql = '''
  ...
  return query(pool, sql, parameters).then((rows) {
    if(rows.length != 1) {
      ...
      Row row = rows.first;
      return new adaheads_model.Contact.fromDB(row.id, row.full_name, row.contact_type, row.enabled);
    }
  });
}

Future<List<adaheads_model.Contact>> _getContactList(Pool pool) {
  String sql = '''
    SELECT id, full_name, contact_type, enabled
    FROM contacts
  ''';
  ...
  return query(pool, sql).then((rows) {
    List<adaheads_model.Contact> contacts = new List<adaheads_model.Contact>();
    for(var row in rows) {
      contacts.add(new adaheads_model.Contact.fromDB(row.id, row.full_name, row.contact_type, row.enabled));
    }
    return contacts;
  });
}
